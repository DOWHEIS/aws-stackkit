...
async function getPool() {
    ...
    pool = new Pool({
        ...
        ...(process.env.SDK_DEV_SERVER === '1' ? {} : { ssl: { rejectUnauthorized: false } }),
    })
    return pool
}
...
export async function insert<T extends Record<string, any>>(table: string, values: T): Promise<void> {
...
export async function selectOne<T>(sql: string, params: Record<string, any> = {}): Promise<T | null> {
    const rows = await selectAll<T>(sql, params)
    return rows[0] ?? null
}

export async function selectAll<T>(sql: string, params: Record<string, any> = {}): Promise<T[]> {
